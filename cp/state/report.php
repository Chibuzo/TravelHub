...
$state_parks = $parkModel->getTravelParksByState($travel_id, $_SESSION['state_id']);

if (isset($_POST['submit'])) {
	$park_id = !empty($_POST['park_id']) ? $_POST['park_id'] : null;
}
$date = isset($_POST['travel_date']) ? $_POST['travel_date'] : date('Y-m-d');
$reports = $report->getDailyReport($date, $travel_id, $_SESSION['state_id'], $park_id);
									...
									<div class="row">
										<div class="col-md-2">
											<select class="form-control" name="park_id">
												...
												<?php
												foreach ($state_parks AS $park) {
													echo "<option value='{$park->park_id}'>{$park->park}</option>";
												}
												?>
											</select>
										</div>
										<div class="col-md-2">
											<select class="form-control" name="report_type">
												...
												<?php
												$_dest_states = Utility::superUnique($dest_states);
												foreach ($_dest_states AS $state) {
													echo "<option value='{$state['id']}'>{$state['dest_state']}</option>";
												}
												?>
											</select>
										</div>
